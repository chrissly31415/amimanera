...
def loadData(nsamples=-1,verbose=False,useRdata=False,useFrequencies=False,concat=True,balance=False,bagofwords=None,bagofwords_v2_0=None,endform=True,comptypes=0,logtransform=None,NA_filler=0,skipLabelEncoding=None,useTubExtended=False,encodeKeepNumber=False):
	...
	if bagofwords_v2_0 is not None:
	    ...
	    vectorizer = CountVectorizer(min_df=3,  max_features=100, lowercase=True,analyzer="word",ngram_range=(1,1),stop_words=None,strip_accents='unicode',token_pattern=r'\w{1,}')
	    ...
	    #discarding columns
	    for col in Xall.columns:
		    if col.startswith('component_id') or col.startswith('quantity_') or col.startswith('doc_component_id') or col.startswith('spec'):
			print "Dropping:",col
	...
	if concat or bagofwords is not None:
	    ...
	    Xall['doc_component_id'] = Xall.apply(lambda x:'%s %s %s %s %s %s %s %s' % (x['component_id_1'],x['component_id_2'],x['component_id_3'],x['component_id_4'],x['component_id_5'],x['component_id_6'],x['component_id_7'],x['component_id_8']),axis=1)
	    Xall['doc_spec'] = Xall.apply(lambda x:'%s %s %s %s %s %s %s %s' % (x['spec1'],x['spec2'],x['spec3'],x['spec4'],x['spec5'],x['spec6'],x['spec7'],x['spec8']),axis=1)
		...
		for col in bagcolumns:
		    vectorizer = CountVectorizer(min_df=2,  max_features=100, lowercase=True,analyzer="word",ngram_range=(1,1),stop_words=None,strip_accents='unicode',token_pattern=r'\w{1,}')
	...
	b=['supplier', 'bracket_pricing', 'material_id', 'end_a_1x', 'end_a_2x', 'end_x_1x', 'end_x_2x', 'end_a', 'end_x', 'component_id_1', 'component_id_2', 'component_id_3', 'component_id_4', 'component_id_5', 'component_id_6', 'component_id_7', 'component_id_8']
	...
	Xall['year'] = Xall.quote_date.dt.year
	Xall['month'] = Xall.quote_date.dt.month
	Xall.drop(['cost','component_id_7','quantity_7','component_id_8','quantity_8','quote_date','spec9','spec10'], axis = 1,inplace=True)
...
if __name__=="__main__":
    ...
    new_feats2=['deltacost', 'cost_per_period', 'quantity_sum_supp', 'annual_usage_sum_supp', 'diameter_sum_supp', 'quantity_size_supp', 'quantity_mean_supp', 'annual_usage_mean_supp', 'diameter_mean_supp', 'quantity_sum_mat', 'annual_usage_sum_mat', 'diameter_sum_mat', 'quantity_size_mat', 'quantity_mean_mat', 'annual_usage_mean_mat', 'diameter_mean_mat']
