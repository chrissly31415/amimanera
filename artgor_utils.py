...
def train_model_regression(X, X_test, y, params, folds, cv_labels=None, model_type='lgb', eval_metric='mae', columns=None,
    ...
    if cv_labels is None:
        cv_gen = folds.split(X,y)
    else:
        cv_gen = folds.split(X,y,cv_labels)

    # split and train on folds
    for fold_n, (train_index, valid_index) in enumerate(cv_gen):
    ...
    if model_type == 'lgb':
        if plot_feature_importance:
            ...
            cols = feature_importance[["feature", "importance"]].groupby("feature").mean().sort_values(
                by="importance", ascending=False)[:n_max].index

            best_features = feature_importance.loc[feature_importance.feature.isin(cols)]
            best_features_plot = best_features.iloc[:50]

            plt.figure(figsize=(16, 12));
            sns.barplot(x="importance", y="feature", data=best_features_plot.sort_values(by="importance", ascending=False));
            plt.title('LGB Features (avg over folds)');

            result_dict['feature_importance'] = best_features.groupby("feature").mean().sort_values(by="importance", ascending=False)
