...
def prepareDatasets(nsamples=-1,onlySpectra=False,deleteSpectra=False,plotting=False,standardize=True,doPCA=5,findPeaks=None,makeDerivative=False,featureFilter=None,loadFeatures=None,deleteFeatures=None,removeVar=0.0,removeCor=0.99):
...
def buildmodels(lmodels,lX,lymat,fit_params=None,scoring='mean_squared_error',cv_split=8,n_jobs=8,gridSearch=False):
	...
	if gridSearch is True:
	    ...
	    #parameters = {'filter__percentile': [100],'model__alpha': [0.001,0.0005],'model__loss':['huber'],'model__penalty':['elasticnet'],'model__epsilon':[1.0],'model__n_iter':[150,200,250]}#pipeline
	    ...
	    parameters = {'filter__param': [100,99,98]}#pipeline
...
if __name__=="__main__":
    ...
    findPeaks=None
    #findPeaks='load'
    makeDerivative=True#CHECK indices after derivative making...
    ...
    #spectra = [m for m in list(training.columns) if m[0]=='m']alphas=[ 0.05,0.1]
    ...
    (Xtrain,Xtest,ymat) = prepareDatasets(nsamples,onlySpectra,deleteSpectra,plotting,standardize,doPCA,findPeaks,makeDerivative,featureFilter,loadFeatures,deleteFeatures,removeVar,removeCor)
	...
	#model = Pipeline([('filter', GenericUnivariateSelect(f_regression, param=99,mode='percentile')), ('model', ElasticNet(alpha=1.0,l1_ratio=0.15))])
	model = Pipeline([('filter', GenericUnivariateSelect(f_regression, param=99,mode='percentile')), ('model', RidgeCV(alphas=[0.1]))])
    ...
    #make the training
    models = buildmodels(models,Xtrain,ymat,cv_split=40,gridSearch=True)
    for i in range(nt):
